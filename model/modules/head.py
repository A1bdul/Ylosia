...
class DetectionHead(nn.Module):
    ...
    def __init__(self, num_classes: int = 80, in_channels: List[int] = []):
        super().__init__()
        self.nc = num_classes
    ...
    def forward(self, x):
        for i in range(self.n_layers):
            x[i] = torch.cat((self.box_conv[i](x[i]), self.cls_convs[i](x[i])), dim=1)
    ...
    def _bias_init(self) -> None:
        ...
        for b_list, c_list, s in zip(self.box_conv, self.cls_convs, self.stride):
