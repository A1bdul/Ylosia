...
from model.modules.utils import make_anchors, dist2bbox, bbox2dist, xywh2xyxy
...
class BaseLoss:
    """Base class for YOLOv8 losses."""

    def __init__(self, device: str):
        """Initializes the loss function with the device."""
        self.device = device
...
class BboxLoss(BaseLoss):
    ...
    def __init__(self, reg_max: int, device: str, use_dfl: bool = False):
        """Initializes the bounding box loss function."""
        super().__init__(device)

        self.reg_max = reg_max
        self.use_dfl = use_dfl

    def compute_loss(self, pred_box_dist: torch.Tensor,
                     pred_boxes: torch.Tensor, target_boxes: torch.Tensor,
                     anchor_points: torch.Tensor, target_scores: torch.Tensor,
                     mask: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """Calculates the bounding box loss components: iou_loss and dfl_loss."""
...
class DetectionLoss(BaseLoss):
    ...
    def __init__(self, model, device: str):
        """Initializes the detection loss function."""
        super().__init__(device)

        detect_head = model.model[-1]

        self.nc = detect_head.nc  # Number of classes
        self.n_outputs = detect_head.n_outputs  # Number of output layers
        self.reg_max = detect_head.reg_max  # Maximum distance for DFL
        self.stride = detect_head.stride  # Strides for each output layer

        self.loss_gains = model.loss_gains  # Loss weighting factors
        ...
        self.tal_assigner = TaskAlignedAssigner(topk=10,
                                                num_classes=self.nc,
                                                alpha=0.5,
    ...
    def decode_bbox(self, anchor_points: torch.Tensor,
                    pred_box_dist: torch.Tensor):
        """Decodes bounding box coordinates from anchor points and predicted box distribution."""
    ...
    def compute_loss(self, batch: Dict[str, torch.Tensor],
        ...
        im_size = torch.tensor(preds[0].shape[2:],
                               device=self.device) * self.stride[0]
        anchor_points, stride_tensor = make_anchors(preds, self.stride)
        pred_boxes = self.decode_bbox(anchor_points,
                                      pred_box_dist)  # (b, h*w, 4) in (xyxy)
        ...
        targets = self.preprocess(targets,
                                  batch_size,
                                  scale_tensor=im_size[[1, 0, 1, 0]])
        # cls, xyxy box

        gt_cls, gt_boxes = targets.split((1, 4), dim=2)
        gt_mask = gt_boxes.sum(
            dim=2, keepdim=True
        ) > 0  # mask to filter out (0,0,0,0) boxes (just used to pad tensor)

        _, target_boxes, target_scores, mask = self.tal_assigner(
            pred_cls.detach().sigmoid(),
            pred_boxes.detach() * stride_tensor, anchor_points * stride_tensor,
            gt_cls, gt_boxes, gt_mask)

        cls_loss = self.bce_loss(pred_cls, target_scores).sum() / max(
            target_scores.sum(), 1)

        if mask.sum() > 0:
            iou_loss, dfl_loss = self.bbox_loss.compute_loss(
                pred_box_dist, pred_boxes, target_boxes / stride_tensor,
                anchor_points, target_scores, mask)
        else:
            iou_loss = torch.tensor(0.0).to(self.device)
            dfl_loss = torch.tensor(0.0).to(self.device)

        loss = self.loss_gains['cls'] * cls_loss + self.loss_gains[
            'iou'] * iou_loss + self.loss_gains['dfl'] * dfl_loss
