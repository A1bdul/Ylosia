...
from model.modules.utils import non_max_suppression, validate_detections_fields, xyxy2xywh
from model.dataset.utils import unpad_xyxy
...
class Detections:
    ...
    def view(self, image: np.ndarray, classes_dict: Dict[int, str] = None, cmap=None, num_classes: int = None,  pads:Tuple[int,int,int,int]=None) -> np.ndarray:
        # self.unpad_xyxy(pads)
        ...
        for j in range(self.__len__()):
            ...
            # draw bounding box
            cv2.rectangle(image, (x2, y2), (x1, y1), cls_color, 20)
            print(f'pred {j}: {label}, {x1}, {y1}, {x2}, {y2}')
            # draw label
            (w, h), _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.6, 1)
            y1_label = y1 - h if y1 - h > 0 else y1 + h
