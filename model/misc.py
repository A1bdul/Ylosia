...
def parse_from_config(config_dict:dict):
    ...
    for i, (module_name, f, r, args) in enumerate(config_dict['backbone'] + config_dict['head']):
        module = getattr(torch.nn, module_name[3:]) if module_name.startswith('nn.') else globals()[module_name]
        if module in (Conv, C2f, SPPF):
            in_channels = channels[f] if isinstance(f, int) else sum([channels[idx] for idx in f])
            ...
            if module == C2f:
                args = [in_channels, out_channels, max(round(r * depth), 1), *args[1:]]
            else:
